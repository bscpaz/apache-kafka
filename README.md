...
#### Topics
Topics, in Kafka, is the channel that producers send messages and consumres read then and we can have many consumers reading from the same topic and reanding the same event. This is different from RabbitMQ which once a message got read from a consumer no one can read it again.
